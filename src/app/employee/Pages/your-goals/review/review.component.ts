...
import { CommentComponent } from "src/app/administration/pages/perfomance-management-module/evaluation/goal/comment/comment.component";
import { ReplyComponent } from "src/app/administration/pages/perfomance-management-module/evaluation/goal/reply/reply.component";
  ...
  this.subscription = this.goalAPI.getGoalDetailId(this.goal_id).subscribe( res=>{
    ...
    if(this.data.reviewenabled='true'){
      ...
      this.subscription = this.reviewAPI.getLastReviewBygoalId(params).subscribe( res=>{
        ...
        console.log("these are comments", this.comments)
        ...
        for(let result of this.comments){
          ...
          this.subscription = this.employeeAPI.getEmployeeId(this.employee_id).subscribe(res=>{
            this.commentedBy = res.firstName
          }, err=>{

          })
          // get each reply
          this.allReplies = result.pm_replies
          for(let resReply of this.allReplies){
            this.employee_id =  resReply.employee_id
            console.log("employee id", this.employee_id )
           this.employeeAPI.getEmployeeId(this.employee_id).subscribe(data=>{
              console.log("employee id", data )
              this.repliedBy = data.firstName
              console.log("replied", this.repliedBy )
        ...
        console.log("replies are", res)
        console.log("replies are", res.pm_comments)
        console.log("replies are", res.pm_comments.pm_replies)
