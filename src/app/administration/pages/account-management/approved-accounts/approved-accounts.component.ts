...
@Component({
  ...
  styleUrls: ['./approved-accounts.component.scss']
})
export class ApprovedAccountsComponent implements  OnInit, OnDestroy {
  rolesArray: any = [
    'ROLE_ADMIN',
	  'ROLE_USER',
    'ROLE_DIRECTOR',
  ...
  getApprovedAccounts(){
    this.authAPI.GetAllApprovedAccounts().subscribe( data =>{
  ...
  ngOnDestroy(): void {
    ...
    onRevoke(email: any){
      if (window.confirm('Are you sure you want to revoke this employee account?')) {     const params = new HttpParams()
        .set('username',email)
        this.authAPI.revokeEmployee(params).subscribe( res=>{
          this._snackBar.open("Role Successfully Revoked!", "X", {
            horizontalPosition: this.horizontalPosition,
            verticalPosition: this.verticalPosition,
            duration: 3000,
            panelClass: ['green-snackbar','login-snackbar'],
